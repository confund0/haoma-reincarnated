...
const editWindowSeconds = 24 * 60 * 60
...
func (a *App) cmdEdit() {
	...
	if !strings.HasPrefix(front, "chat:") {
		a.log("[red]/edit[white] must be used inside a chat window")
		return
	}
	...
	a.winMu.Unlock()
	if cp == nil {
		a.log("[red]/edit[white] no active chat page")
		return
	}
	cands := collectEditable(cp)
	if len(cands) == 0 {
		a.log("[yellow]/edit[white] no editable messages (own text, last 24h)")
		return
	}
	a.showEditPicker(cp, cands)
}

func collectEditable(cp *chatPage) []editCandidate {
...
func (a *App) showEditPicker(cp *chatPage, cands []editCandidate) {
	const pageName = "edit-picker"
	...
	flex.SetBorder(true).SetTitle(fmt.Sprintf(" edit message — %d candidate(s) ", len(cands)))
...
func (a *App) showEditText(cp *chatPage, c editCandidate) {
	const pageName = "edit-text"

	form := tview.NewForm()
	form.SetButtonsAlign(tview.AlignCenter)

	var buf string = c.text
	form.AddInputField("New text", c.text, 60, nil, func(v string) { buf = v })
	...
	form.AddButton("Save", func() {
		newText := strings.TrimSpace(buf)
		if newText == "" {
			a.log("[yellow]edit[white] empty text — cancelled")
			dismiss()
			return
		}
		if newText == c.text {
			a.log("[yellow]edit[white] no change — cancelled")
			dismiss()
			return
		}
		peerID := cp.peerID
		dismiss()
		a.doSendEdit(peerID, c.msgID, newText)
	})
	...
	form.SetBorder(true).SetTitle(" edit message ")
	grid := tview.NewGrid().
		SetColumns(0, 80, 0).
		SetRows(0, 7, 0).
...
func (a *App) doSendEdit(peerID, targetMsgID, text string) {
	if a.peerRetiredAt(peerID) != 0 {
		a.log("[red]peer retired[white] — edit not sent")
		return
	}
	a.sendRequest(ipc.FrameSendEdit, ipc.SendEditRequest{
		PeerID:      peerID,
		...
		if f.Type != ipc.FrameEditSent {
			a.log("[red]/edit[white] unexpected response: %s", f.Type)
			return
		}
	})
}
